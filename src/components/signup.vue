    ...
    <div class="row vertical-10p">
      <div class="container">
        ...
        <div class="text-center col-md-4 col-sm-offset-4">
          ...
          <div v-for="error in errors" class="text-red">{{error | capitalize}}</div>
          <div v-if=message class="text-green"><p>{{message | capitalize }}</p></div>
          <!-- login form -->
          <form class="ui form loginForm" @submit.prevent="signUp">

            <div class="input-group">
              <span class="input-group-addon"><i class="fa fa-envelope"></i></span>
              <input class="form-control" name="username" placeholder="Email" type="text" v-model="email">
            </div>
            <div class="input-group">
              <span class="input-group-addon"><i class="fa fa-envelope"></i></span>
            ...
            <div class="input-group">
              <span class="input-group-addon"><i class="fa fa-lock"></i></span>
              <input class="form-control" name="password" placeholder="First Name" type="text"
                     v-model="firstName">
            </div>
            <div class="input-group">
              <span class="input-group-addon"><i class="fa fa-lock"></i></span>
              <input class="form-control" name="password" placeholder="Last Name" type="text"
                     v-model="lastName">
            </div>
            <button type="submit" class="btn btn-primary btn-lg" :class="loading">Sign Up</button>
          </form>
            <div class="m-t-20">
              <a v-link="'login'"class="btn btn-primary btn-lg m-t-5">Login</a>
            </div>
        </div>
      </div>
    </div>
    ...
    methods: {
      signUp() {
        ...
        if (this.password === this.confirmPassword) {
          this.$parent.callAPI('POST', baseUrl + '/api/accounts/create', this.userModel).then(
            function (response) {
                self.message='Registration successful'
                self.$router.go('/login')
            },
            function (response) {
             self.handleSignUpError(response,store)
            })
      ...
      handleSignUpError(response, store){
        store.dispatch('TOGGLE_LOADING')
        ...
        if (response.data) {
          ...
          for (var property in modelState) {
            if (modelState.hasOwnProperty(property)) {
              this.errors.push(modelState[property])
            }
          }
        }
